...
void Heap::initialize(){
        ...
        printf("build a heap,size is 40960B,from %x to %x.\n",startAddr,tail);
}
...
int HeapManager::malloc(int byte){
    ...
    heap.startAddr+=byte;
    heap.shengyu-=byte;
    printf("malloc from %x to %x, now from %x to %x is available.\n",heap.startAddr-byte,heap.startAddr,heap.startAddr,heap.tail);
...
void HeapManager::free(int byte){
    ...
    heap.startAddr-=byte;
    heap.shengyu+=byte;
    printf("free   from %x to %x, now from %x to %x is available.\n",heap.startAddr,heap.startAddr+byte,heap.startAddr,heap.tail);
}
