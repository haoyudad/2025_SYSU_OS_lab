...
public:
    ...
    node a[100];
    node *kerhead;//指向内核fifo队列的头节点
    node *kertail;//指向内核fifo队列的尾节点
...
public:
    ...
    // 页内存释放
    int  releasePages(enum AddressPoolType type, const int virtualAddress,const int beforeSize, const int count); 
    //把函数返回值改成int型，可以对释放后的首地址作出修改   
